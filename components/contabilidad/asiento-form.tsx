...
import { createAsientoContable } from "@/app/actions/contabilidad"
...
export function AsientoForm({ cuentas }: { cuentas: CuentaMovimiento[] }) {
  ...
  const actualizarDetalle = (index: number, campo: keyof DetalleAsientoInput, valor: any) => {
          ...
          <Table>
            ...
            <TableBody>
                ...
                <TableRow key={index}>
                  <TableCell>
                    <Select
                      ...
                      onValueChange={(value) => actualizarDetalle(index, "cuentaId", value)}
                      required
                    >
                      <SelectTrigger>
                  ...
                  <TableCell>
                    {detalles.length > 1 && (
                      <Button type="button" variant="ghost" size="sm" onClick={() => eliminarDetalle(index)}>
              ...
              <TableRow className="bg-slate-50 font-semibold">
                ...
                <TableCell className="text-right">${totalDebe.toFixed(2)}</TableCell>
                <TableCell className="text-right">${totalHaber.toFixed(2)}</TableCell>
                <TableCell></TableCell>
              </TableRow>
              {Math.abs(diferencia) > 0.01 && (
                <TableRow>
                  <TableCell colSpan={5} className="text-center text-red-600">
                    ⚠️ El asiento no está cuadrado. Diferencia: ${Math.abs(diferencia).toFixed(2)}
                  </TableCell>
                </TableRow>
              )}
            </TableBody>
